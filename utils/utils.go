...
var (
	logger = CelestiaKyveRpcLogger("utils")
)
...
func GetFromUrl(url string) ([]byte, error) {
	...
	if version != "" {
		...
		request.Header.Set("User-Agent", fmt.Sprintf("celestia-kyve-rpc/%v (%v / %v / %v)", version, runtime.GOOS, runtime.GOARCH, runtime.Version()))
	} else {
		request.Header.Set("User-Agent", fmt.Sprintf("celestia-kyve-rpc/dev (%v / %v / %v)", runtime.GOOS, runtime.GOARCH, runtime.Version()))
	}
